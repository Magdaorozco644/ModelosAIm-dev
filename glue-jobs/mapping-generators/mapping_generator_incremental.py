...
BUCKETSCRIPTS = "viamericas-datalake-dev-us-east-1-283731589572-glue-jobs/incremental/scripts"
...
incremental_updates = [
    ...
    'accounting_submittransaction', 'vcw_billpayment_sales', 'comision_agent_modo_pago_grupo', 'forex_feed_market', 'vcw_moneyorders_sales', 'vcw_billpayment_viaone_sales', 'vcw_sales_products', 'vcw_states_pricing', 'receiver_gp_components', 'checkverification', 'customers_customer', 'viacheckfeaturemetrics', 'rate_group_agent', 'branch', 'returnchecks'
]
...
for index, row in df.iterrows():
    ...
    if table in incremental_updates: 
        state_machine_name = f'sdlf_sm_incremental_{database}_{schema}_{table}' 

        # path raw files and path scripts
        path_files =  f's3://{BUCKETRAW}/{database}/{schema}/{table}'
        path_script = f"s3://{BUCKETSCRIPTS}/gjdev_incremental_{database}_{schema}_{table}_glue_script.py"
            
        # names
        job_name = f'sdlf_gj_incremental_{database}_{schema}_{table}'
        ...
        mapping = {
            'ObjectName': f'{database}{schema}{table}Incremental',
            'JobName': job_name,
        ...
        all +=  f'{database}{schema}{table}Incremental, '
...
for mapping in mappers:
    ...
    with open('mapping_incremental.txt', 'a') as crw_file:
