...
df = wr.athena.read_sql_query(sql="select coalesce(cast(max(FEED_DATE) as varchar), cast(date_add('hour', -5, from_unixtime(cast(to_unixtime(current_timestamp) AS bigint))) as varchar)) as max_date from viamericas.forex_feed_market", database="viamericas")
...
qryStr = f"(SELECT [FEED_BID],[FEED_SOURCE],[FEED_ASK],[FEED_REMAINING],[FEED_ID],[SYMBOL],[FEED_PRICE],[FEED_DATE],[FEED_TIME], convert(date, [FEED_DATE]) as day FROM envio.dba.forex_feed_market WHERE  [FEED_DATE] >= '{today} 00:00:00') x"
