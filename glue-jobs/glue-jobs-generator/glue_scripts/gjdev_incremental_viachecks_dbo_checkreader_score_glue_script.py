...
df = wr.athena.read_sql_query(sql="select coalesce(cast(max(system_date) as varchar), cast(date_add('hour', -5, from_unixtime(cast(to_unixtime(current_timestamp) AS bigint))) as varchar)) as max_date from viamericas.checkreader_score", database="viamericas")
...
qryStr = f"(SELECT [RESULT],[CHECK_SCORE_ID],[SCORE],[CHECKPROPERTYID],[CDCHEQUE],[VCLCHECKBATCHID],[CDMAKER],[CHECKID],[SYSTEM_DATE], convert(date, [system_date]) as day FROM viachecks.dbo.checkreader_score WHERE  [system_date] >= '{today} 00:00:00') x"
