...
df = wr.athena.read_sql_query(sql="select coalesce(cast(max(DATE_CREATED) as varchar), cast(date_add('hour', -5, from_unixtime(cast(to_unixtime(current_timestamp) AS bigint))) as varchar)) as max_date from viamericas.ml_fraud_score", database="viamericas")
...
qryStr = f"(SELECT [SCORE_1],[WAS_FRAUD],[SCORE_2],[MATRIX_CATEGORY],[DATE_CREATED],[ID_RECEIVER],[ID],[SCORE],[DATE_PROCESSED],[IS_PROCESSED],[ID_BRANCH],[IS_AT_VECTOR],[DATE_RECEIVER],[IS_PROCESSING], convert(date, [DATE_CREATED]) as day FROM envio.fraud.ml_fraud_score WHERE  [DATE_CREATED] >= '{today} 00:00:00') x"
