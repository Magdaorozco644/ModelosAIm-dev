...
df = wr.athena.read_sql_query(sql="select coalesce(cast(max(processeddate) as varchar), cast(date_add('hour', -5, from_unixtime(cast(to_unixtime(current_timestamp) AS bigint))) as varchar)) as max_date from viamericas.historicalonholdrelease", database="viamericas")
...
qryStr = f"(SELECT [HoldDate],[IdHoldGroup],[idVoidReason],[Review],[idVoidSubReason],[ProcessedDate],[CheckStatus],[HistoricalId],[ProcessedUser],[CheckId],[ReviewDate], convert(date, [processeddate]) as day FROM viachecks.viacheck.historicalonholdrelease WHERE  [processeddate] >= '{today} 00:00:00') x"
