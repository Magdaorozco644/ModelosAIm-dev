...
df = wr.athena.read_sql_query(sql="select coalesce(cast(max(MetricDate) as varchar), cast(date_add('hour', -5, from_unixtime(cast(to_unixtime(current_timestamp) AS bigint))) as varchar)) as max_date from viamericas.viacheckfeaturemetrics", database="viamericas")
...
qryStr = f"(SELECT [IdFeature],[MetricDate],[Agency],[Routing],[IdVerification],[IdFeatureModule],[UserName],[CheckNumber],[Amount],[Account], convert(date, [MetricDate]) as day FROM viachecks.viacheck.viacheckfeaturemetrics WHERE  [MetricDate] >= '{today} 00:00:00') x"
