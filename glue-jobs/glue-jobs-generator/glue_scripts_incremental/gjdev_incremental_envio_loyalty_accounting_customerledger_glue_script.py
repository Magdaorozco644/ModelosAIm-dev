...
max_date = get_partitions('viamericas','accounting_customerledger')[0][0]
print("Max date in partitions is:", max_date)

# Get max date in athena

df = wr.athena.read_sql_query(sql=f"select coalesce(cast(max(CREATION_DATE) as varchar), substring(cast(at_timezone(current_timestamp,'America/Bogota') as varchar(100)),1,23)) as max_date from viamericas.accounting_customerledger where day = '{max_date}' and CREATION_DATE <= cast(substring(cast(at_timezone(current_timestamp,'America/Bogota') as varchar(100)),1,23) as timestamp)", database="viamericas")
...
qryStr = f"(SELECT [creation_date],[id],rtrim([customer_id]) as customer_id,[balance],rtrim([description]) as description,[spending_rule_id],[earning_rule_id],[total_points],[journal_id],[company_id],[cost],[points], convert(date, [CREATION_DATE]) as day FROM envio.loyalty.accounting_customerledger WHERE [CREATION_DATE] >= '{athena_max_date}') x"
...
if number_of_rows > 0:
    ...
    for day, count in thelist:
        if count > 0:
            ...
            totaldfpre = spark.sql(f" select t2.* from viamericas.accounting_customerledger t2 where day = '{day}'")
            totaldfpre.unionByName(dfincremental).dropDuplicates()
  
            # Definir la ruta de salida en S3
            
            s3_output_path = f"s3://{secret_bucket_names['BUCKET_RAW']}/envio/loyalty/accounting_customerledger/"
            # Escribir el DataFrame en formato Parquet en S3
            totaldfpre.write.partitionBy("day").parquet(s3_output_path, mode="overwrite")    
            print(f'Data for: {day} written succesfully')
            partition_creator_v2('viamericas','accounting_customerledger', {'df': None, 'PartitionValues': tuple([str({day})])})
            abcdefg
else:
