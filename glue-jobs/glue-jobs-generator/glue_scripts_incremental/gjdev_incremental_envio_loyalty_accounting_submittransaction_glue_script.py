...
max_date = get_partitions('viamericas','accounting_submittransaction')[0][0]
print("Max date in partitions is:", max_date)

# Get max date in athena

df = wr.athena.read_sql_query(sql=f"select coalesce(cast(max(MODIFICATION_DATE) as varchar), substring(cast(at_timezone(current_timestamp,'America/Bogota') as varchar(100)),1,23)) as max_date from viamericas.accounting_submittransaction where day = '{max_date}' and MODIFICATION_DATE <= cast(substring(cast(at_timezone(current_timestamp,'America/Bogota') as varchar(100)),1,23) as timestamp)", database="viamericas")
...
qryStr = f"(SELECT [is_cancelled],[creation_date],[modification_date],rtrim([id]) as id, convert(date, [MODIFICATION_DATE]) as day FROM envio.loyalty.accounting_submittransaction WHERE [MODIFICATION_DATE] >= '{athena_max_date}') x"
...
if number_of_rows > 0:
    ...
    for day, count in thelist:
        if count > 0:
            ...
            totaldfpre = spark.sql(f" select t2.* from viamericas.accounting_submittransaction t2 where day = '{day}'")
            totaldfpre.unionByName(dfincremental).dropDuplicates()
  
            # Definir la ruta de salida en S3
            
            s3_output_path = f"s3://{secret_bucket_names['BUCKET_RAW']}/envio/loyalty/accounting_submittransaction/"
            # Escribir el DataFrame en formato Parquet en S3
            totaldfpre.write.partitionBy("day").parquet(s3_output_path, mode="overwrite")    
            print(f'Data for: {day} written succesfully')
            partition_creator_v2('viamericas','accounting_submittransaction', {'df': None, 'PartitionValues': tuple([str({day})])})
            abcdefg
else:
