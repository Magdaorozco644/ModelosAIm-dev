...
max_date = get_partitions('viamericas','receiver_gp_components')[0][0]
print("Max date in partitions is:", max_date)

# Get max date in athena

df = wr.athena.read_sql_query(sql=f"select coalesce(cast(max(DATE_RECEIVER) as varchar), substring(cast(at_timezone(current_timestamp,'America/Bogota') as varchar(100)),1,23)) as max_date from viamericas.receiver_gp_components where day = '{max_date}' and DATE_RECEIVER <= cast(substring(cast(at_timezone(current_timestamp,'America/Bogota') as varchar(100)),1,23) as timestamp)", database="viamericas")
...
qryStr = f"(SELECT [ID_BRANCH],[VIADEAL_ESTIMATED],[DATE_RECEIVER],[VIADEAL_REAL],[ID_RECEIVER], convert(date, [DATE_RECEIVER]) as day FROM envio.crm.receiver_gp_components WHERE [DATE_RECEIVER] >= '{athena_max_date}') x"
...
jdbcDF = spark.read.format('jdbc')\
    ...
    .option('partitionColumn', 'DATE_RECEIVER')\
...
if number_of_rows > 0:
    ...
    for day, count in thelist:
        if count > 0:
            ...
            totaldfpre = spark.sql(f" select t2.* from viamericas.receiver_gp_components t2 where day = '{day}'")
            totaldfpre.unionByName(dfincremental).dropDuplicates()
  
            # Definir la ruta de salida en S3
            
            s3_output_path = f"s3://{secret_bucket_names['BUCKET_RAW']}/envio/crm/receiver_gp_components/"
            # Escribir el DataFrame en formato Parquet en S3
            totaldfpre.write.partitionBy("day").parquet(s3_output_path, mode="overwrite")    
            print(f'Data for: {day} written succesfully')
            partition_creator_v2('viamericas','receiver_gp_components', {'df': None, 'PartitionValues': tuple([str({day})])})
            abcdefg
else:
