...
max_date = get_partitions('viamericas','ml_fraud_score')[0][0]
print("Max date in partitions is:", max_date)

# Get max date in athena

df = wr.athena.read_sql_query(sql=f"select coalesce(cast(max(DATE_CREATED) as varchar), substring(cast(at_timezone(current_timestamp,'America/Bogota') as varchar(100)),1,23)) as max_date from viamericas.ml_fraud_score where day = '{max_date}' and DATE_CREATED <= cast(substring(cast(at_timezone(current_timestamp,'America/Bogota') as varchar(100)),1,23) as timestamp)", database="viamericas")
...
qryStr = f"(SELECT [IS_PROCESSED],[DATE_CREATED],[DATE_PROCESSED],[IS_PROCESSING],[IS_AT_VECTOR],rtrim([ID_BRANCH]) as ID_BRANCH,[MATRIX_CATEGORY],[SCORE_1],[WAS_FRAUD],[DATE_RECEIVER],[SCORE],[ID_RECEIVER],[ID],[SCORE_2], convert(date, [DATE_CREATED]) as day FROM envio.fraud.ml_fraud_score WHERE [DATE_CREATED] >= '{athena_max_date}') x"
...
if number_of_rows > 0:
    ...
    for day, count in thelist:
        if count > 0:
            ...
            totaldfpre = spark.sql(f" select t2.* from viamericas.ml_fraud_score t2 where day = '{day}'")
            totaldfpre.unionByName(dfincremental).dropDuplicates()
  
            # Definir la ruta de salida en S3
            
            s3_output_path = f"s3://{secret_bucket_names['BUCKET_RAW']}/envio/fraud/ml_fraud_score/"
            # Escribir el DataFrame en formato Parquet en S3
            totaldfpre.write.partitionBy("day").parquet(s3_output_path, mode="overwrite")    
            print(f'Data for: {day} written succesfully')
            partition_creator_v2('viamericas','ml_fraud_score', {'df': None, 'PartitionValues': tuple([str({day})])})
            abcdefg
else:
