...
max_date = get_partitions('viamericas','vcw_billpayment_viaone_sales')[0][0]
print("Max date in partitions is:", max_date)

# Get max date in athena

df = wr.athena.read_sql_query(sql=f"select coalesce(cast(max(CREATION_DATE) as varchar), substring(cast(at_timezone(current_timestamp,'America/Bogota') as varchar(100)),1,23)) as max_date from viamericas.vcw_billpayment_viaone_sales where day = '{max_date}' and CREATION_DATE <= cast(substring(cast(at_timezone(current_timestamp,'America/Bogota') as varchar(100)),1,23) as timestamp)", database="viamericas")
...
qryStr = f"(SELECT [FUNDING],[LASTFOUR],[TOTAL_AMOUNT],rtrim([ID_PAYMENT]) as ID_PAYMENT,[UUID_SENDER],[ID_BRANCH],[CUSTOMER_CODE],[CREATION_DATE],[AUTH_CODE],[ID_BILLPAYMENT_VIONE_SALES], convert(date, [CREATION_DATE]) as day FROM envio.dba.vcw_billpayment_viaone_sales WHERE [CREATION_DATE] >= '{athena_max_date}') x"
...
if number_of_rows > 0:
    ...
    for day, count in thelist:
        if count > 0:
            ...
            totaldfpre = spark.sql(f" select t2.* from viamericas.vcw_billpayment_viaone_sales t2 where day = '{day}'")
            totaldfpre.unionByName(dfincremental).dropDuplicates()
  
            # Definir la ruta de salida en S3
            
            s3_output_path = f"s3://{secret_bucket_names['BUCKET_RAW']}/envio/dba/vcw_billpayment_viaone_sales/"
            # Escribir el DataFrame en formato Parquet en S3
            totaldfpre.write.partitionBy("day").parquet(s3_output_path, mode="overwrite")    
            print(f'Data for: {day} written succesfully')
            partition_creator_v2('viamericas','vcw_billpayment_viaone_sales', {'df': None, 'PartitionValues': tuple([str({day})])})
            abcdefg
else:
