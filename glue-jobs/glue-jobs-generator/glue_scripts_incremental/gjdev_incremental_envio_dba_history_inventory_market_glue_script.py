...
max_date = get_partitions('viamericas','history_inventory_market')[0][0]
print("Max date in partitions is:", max_date)

# Get max date in athena

df = wr.athena.read_sql_query(sql=f"select coalesce(cast(max(DATE) as varchar), substring(cast(at_timezone(current_timestamp,'America/Bogota') as varchar(100)),1,23)) as max_date from viamericas.history_inventory_market where day = '{max_date}' and DATE <= cast(substring(cast(at_timezone(current_timestamp,'America/Bogota') as varchar(100)),1,23) as timestamp)", database="viamericas")
...
qryStr = f"(SELECT rtrim([GROUP_TYPE]) as GROUP_TYPE,[viatasa_via],[NEW_PURCHASE],[DATE],[ID_CURRENCY],[DOLLAR],[NUM_ORDERS],[AVG_RATE],[viatasa_agencia],[INFO_TYPE],[PESOS], convert(date, [DATE]) as day FROM envio.dba.history_inventory_market WHERE [DATE] >= '{athena_max_date}') x"
...
if number_of_rows > 0:
    ...
    for day, count in thelist:
        if count > 0:
            ...
            totaldfpre = spark.sql(f" select t2.* from viamericas.history_inventory_market t2 where day = '{day}'")
            totaldfpre.unionByName(dfincremental).dropDuplicates()
  
            # Definir la ruta de salida en S3
            
            s3_output_path = f"s3://{secret_bucket_names['BUCKET_RAW']}/envio/dba/history_inventory_market/"
            # Escribir el DataFrame en formato Parquet en S3
            totaldfpre.write.partitionBy("day").parquet(s3_output_path, mode="overwrite")    
            print(f'Data for: {day} written succesfully')
            partition_creator_v2('viamericas','history_inventory_market', {'df': None, 'PartitionValues': tuple([str({day})])})
            abcdefg
else:
