...
max_date = get_partitions('viamericas','audit_rate_group_agent')[0][0]
print("Max date in partitions is:", max_date)

# Get max date in athena

df = wr.athena.read_sql_query(sql=f"select coalesce(cast(max(DATE_PROCESS) as varchar), substring(cast(at_timezone(current_timestamp,'America/Bogota') as varchar(100)),1,23)) as max_date from viamericas.audit_rate_group_agent where day = '{max_date}' and DATE_PROCESS <= cast(substring(cast(at_timezone(current_timestamp,'America/Bogota') as varchar(100)),1,23) as timestamp)", database="viamericas")
...
qryStr = f"(SELECT rtrim([ID_COUNTRY]) as ID_COUNTRY,rtrim([ID_MAIN_BRANCH]) as ID_MAIN_BRANCH,[DATE_UPGRADE],rtrim([ID_MODO_PAGO]) as ID_MODO_PAGO,[DATE_PROCESS],[TEMP_RATE_MOD],rtrim([ID_CURRENY]) as ID_CURRENY,rtrim([PROCESS]) as PROCESS,[LIMIT_INF_RATE],[ID],rtrim([ID_BRANCH]) as ID_BRANCH,[USER_MODIFY],[LIMIT_SEND_RATE],rtrim([ID_COUNTRY_RATE]) as ID_COUNTRY_RATE,[ID_RATE_TYPE],[ID_CURRENCY_SOURCE],[LIMIT_SUP_RATE],[TEMP_RATE_VALID_THRU],rtrim([ID_STATE]) as ID_STATE,[RATE], convert(date, [DATE_PROCESS]) as day FROM envio.dba.audit_rate_group_agent WHERE [DATE_PROCESS] >= '{athena_max_date}') x"
...
if number_of_rows > 0:
    ...
    for day, count in thelist:
        if count > 0:
            ...
            totaldfpre = spark.sql(f" select t2.* from viamericas.audit_rate_group_agent t2 where day = '{day}'")
            totaldfpre.unionByName(dfincremental).dropDuplicates()
  
            # Definir la ruta de salida en S3
            
            s3_output_path = f"s3://{secret_bucket_names['BUCKET_RAW']}/envio/dba/audit_rate_group_agent/"
            # Escribir el DataFrame en formato Parquet en S3
            totaldfpre.write.partitionBy("day").parquet(s3_output_path, mode="overwrite")    
            print(f'Data for: {day} written succesfully')
            partition_creator_v2('viamericas','audit_rate_group_agent', {'df': None, 'PartitionValues': tuple([str({day})])})
            abcdefg
else:
