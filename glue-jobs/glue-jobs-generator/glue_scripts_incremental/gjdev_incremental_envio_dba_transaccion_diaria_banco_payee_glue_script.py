...
max_date = get_partitions('viamericas','transaccion_diaria_banco_payee')[0][0]
print("Max date in partitions is:", max_date)

# Get max date in athena

df = wr.athena.read_sql_query(sql=f"select coalesce(cast(max(DATE_TRANS_DIARIA) as varchar), substring(cast(at_timezone(current_timestamp,'America/Bogota') as varchar(100)),1,23)) as max_date from viamericas.transaccion_diaria_banco_payee where day = '{max_date}' and DATE_TRANS_DIARIA <= cast(substring(cast(at_timezone(current_timestamp,'America/Bogota') as varchar(100)),1,23) as timestamp)", database="viamericas")
...
qryStr = f"(SELECT [EXCHANGE_RATE],[DATE_TRANS_DIARIA],rtrim([ID_CUENTA_BANCO]) as ID_CUENTA_BANCO,[DEPOSIT_CODE],rtrim([ID_CURRENCY]) as ID_CURRENCY,[ID_PURCHASE],rtrim([ISVALIDATEDWITHBANKS]) as ISVALIDATEDWITHBANKS,[CONS_TRANS_DIARIA],rtrim([ISVALIDATEDWITHPAYER]) as ISVALIDATEDWITHPAYER,[RECONCILIATIONDATE],rtrim([ISUSEDFORSTOCK]) as ISUSEDFORSTOCK,rtrim([ID_BANCO]) as ID_BANCO, convert(date, [DATE_TRANS_DIARIA]) as day FROM envio.dba.transaccion_diaria_banco_payee WHERE [DATE_TRANS_DIARIA] >= '{athena_max_date}') x"
...
if number_of_rows > 0:
    ...
    for day, count in thelist:
        if count > 0:
            ...
            totaldfpre = spark.sql(f" select t2.* from viamericas.transaccion_diaria_banco_payee t2 where day = '{day}'")
            totaldfpre.unionByName(dfincremental).dropDuplicates()
  
            # Definir la ruta de salida en S3
            
            s3_output_path = f"s3://{secret_bucket_names['BUCKET_RAW']}/envio/dba/transaccion_diaria_banco_payee/"
            # Escribir el DataFrame en formato Parquet en S3
            totaldfpre.write.partitionBy("day").parquet(s3_output_path, mode="overwrite")    
            print(f'Data for: {day} written succesfully')
            partition_creator_v2('viamericas','transaccion_diaria_banco_payee', {'df': None, 'PartitionValues': tuple([str({day})])})
            abcdefg
else:
