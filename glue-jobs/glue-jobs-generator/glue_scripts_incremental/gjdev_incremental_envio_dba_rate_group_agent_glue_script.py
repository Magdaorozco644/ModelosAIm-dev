...
max_date = get_partitions('viamericas','rate_group_agent')[0][0]
print("Max date in partitions is:", max_date)

# Get max date in athena

df = wr.athena.read_sql_query(sql=f"select coalesce(cast(max(DATE_UPGRADE) as varchar), substring(cast(at_timezone(current_timestamp,'America/Bogota') as varchar(100)),1,23)) as max_date from viamericas.rate_group_agent where day = '{max_date}' and DATE_UPGRADE <= cast(substring(cast(at_timezone(current_timestamp,'America/Bogota') as varchar(100)),1,23) as timestamp)", database="viamericas")
...
qryStr = f"(SELECT [LIMIT_SEND_RATE],[TEMP_RATE_MOD],rtrim([ID_COUNTRY]) as ID_COUNTRY,rtrim([ID_MODO_PAGO]) as ID_MODO_PAGO,rtrim([ID_CURRENY]) as ID_CURRENY,[DATE_UPGRADE],[RATE],rtrim([ID_COUNTRY_RATE]) as ID_COUNTRY_RATE,rtrim([ID_BRANCH]) as ID_BRANCH,[TEMP_RATE_VALID_THRU],[ID_CURRENCY_SOURCE],rtrim([ID_STATE]) as ID_STATE,[ID_RATE_TYPE],[LIMIT_SUP_RATE],[LIMIT_INF_RATE],[PLUS_WEEKEND],[PLUS_BASE],rtrim([ID_MAIN_BRANCH]) as ID_MAIN_BRANCH, convert(date, [DATE_UPGRADE]) as day FROM envio.dba.rate_group_agent WHERE [DATE_UPGRADE] >= '{athena_max_date}') x"
...
jdbcDF = spark.read.format('jdbc')\
    ...
    .option('partitionColumn', 'DATE_UPGRADE')\
...
if number_of_rows > 0:
    ...
    for day, count in thelist:
        if count > 0:
            ...
            totaldfpre = spark.sql(f" select t2.* from viamericas.rate_group_agent t2 where day = '{day}'")
            totaldfpre.unionByName(dfincremental).dropDuplicates()
  
            # Definir la ruta de salida en S3
            
            s3_output_path = f"s3://{secret_bucket_names['BUCKET_RAW']}/envio/dba/rate_group_agent/"
            # Escribir el DataFrame en formato Parquet en S3
            totaldfpre.write.partitionBy("day").parquet(s3_output_path, mode="overwrite")    
            print(f'Data for: {day} written succesfully')
            partition_creator_v2('viamericas','rate_group_agent', {'df': None, 'PartitionValues': tuple([str({day})])})
            abcdefg
else:
