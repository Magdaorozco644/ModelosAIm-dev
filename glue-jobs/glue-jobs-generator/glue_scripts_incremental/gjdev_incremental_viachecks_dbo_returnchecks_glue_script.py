...
max_date = get_partitions('viamericas','returnchecks')[0][0]
print("Max date in partitions is:", max_date)

# Get max date in athena

df = wr.athena.read_sql_query(sql=f"select coalesce(cast(max(insertedDate) as varchar), substring(cast(at_timezone(current_timestamp,'America/Bogota') as varchar(100)),1,23)) as max_date from viamericas.returnchecks where day = '{max_date}' and insertedDate <= cast(substring(cast(at_timezone(current_timestamp,'America/Bogota') as varchar(100)),1,23) as timestamp)", database="viamericas")
...
qryStr = f"(SELECT [FEE],[returnID],[insertedDate],[returnReason],[CONS_TRANS_DIARIA],[errorIRD],[creditTransactionId],[returnTime],[SNENVIO],[reprocessIRD],[checkID], convert(date, [insertedDate]) as day FROM viachecks.dbo.returnchecks WHERE [insertedDate] >= '{athena_max_date}') x"
...
jdbcDF = spark.read.format('jdbc')\
    ...
    .option('partitionColumn', 'insertedDate')\
...
if number_of_rows > 0:
    ...
    for day, count in thelist:
        if count > 0:
            ...
            totaldfpre = spark.sql(f" select t2.* from viamericas.returnchecks t2 where day = '{day}'")
            totaldfpre.unionByName(dfincremental).dropDuplicates()
  
            # Definir la ruta de salida en S3
            
            s3_output_path = f"s3://{secret_bucket_names['BUCKET_RAW']}/viachecks/dbo/returnchecks/"
            # Escribir el DataFrame en formato Parquet en S3
            totaldfpre.write.partitionBy("day").parquet(s3_output_path, mode="overwrite")    
            print(f'Data for: {day} written succesfully')
            partition_creator_v2('viamericas','returnchecks', {'df': None, 'PartitionValues': tuple([str({day})])})
            abcdefg
else:
