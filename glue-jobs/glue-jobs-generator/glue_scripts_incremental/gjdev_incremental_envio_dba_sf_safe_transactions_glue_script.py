...
max_date = get_partitions('viamericas','sf_safe_transactions')[0][0]
print("Max date in partitions is:", max_date)

# Get max date in athena

df = wr.athena.read_sql_query(sql=f"select coalesce(cast(max(TRANSACTION_DATE) as varchar), substring(cast(at_timezone(current_timestamp,'America/Bogota') as varchar(100)),1,23)) as max_date from viamericas.sf_safe_transactions where day = '{max_date}' and TRANSACTION_DATE <= cast(substring(cast(at_timezone(current_timestamp,'America/Bogota') as varchar(100)),1,23) as timestamp)", database="viamericas")
...
qryStr = f"(SELECT [CREDITED_CONS_TRANS_DIARIA],[CASH_AMOUNT],[USER_ID],[TEST],[MESSAGE],[SAFE_CASSETTE_ID],[ID_SAFE_TRANSACTIONS],[ADDRESSNAME],[TRANSACTION_NUMBER],[BILL2],[ID_SAFE_BRANCH],[MESSAGE_CODE],[BILL5],[BILL10],[BILL50],[BILL1],rtrim([ID_BRANCH]) as ID_BRANCH,[WIRELESSID],[TRANSACTION_TYPE],[RECEIVED_DATE],[AMOUNT],[USER_NAME],[BILL20],[TRANSACTION_DATE],[BILL100], convert(date, [TRANSACTION_DATE]) as day FROM envio.dba.sf_safe_transactions WHERE [TRANSACTION_DATE] >= '{athena_max_date}') x"
...
if number_of_rows > 0:
    ...
    for day, count in thelist:
        if count > 0:
            ...
            totaldfpre = spark.sql(f" select t2.* from viamericas.sf_safe_transactions t2 where day = '{day}'")
            totaldfpre.unionByName(dfincremental).dropDuplicates()
  
            # Definir la ruta de salida en S3
            
            s3_output_path = f"s3://{secret_bucket_names['BUCKET_RAW']}/envio/dba/sf_safe_transactions/"
            # Escribir el DataFrame en formato Parquet en S3
            totaldfpre.write.partitionBy("day").parquet(s3_output_path, mode="overwrite")    
            print(f'Data for: {day} written succesfully')
            partition_creator_v2('viamericas','sf_safe_transactions', {'df': None, 'PartitionValues': tuple([str({day})])})
            abcdefg
else:
