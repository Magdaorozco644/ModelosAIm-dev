...
max_date = get_partitions('viamericas','transaccion_diaria_payee')[0][0]
print("Max date in partitions is:", max_date)

# Get max date in athena

df = wr.athena.read_sql_query(sql=f"select coalesce(cast(max(DATE_TRANS_DIARIA) as varchar), substring(cast(at_timezone(current_timestamp,'America/Bogota') as varchar(100)),1,23)) as max_date from viamericas.transaccion_diaria_payee where day = '{max_date}' and DATE_TRANS_DIARIA <= cast(substring(cast(at_timezone(current_timestamp,'America/Bogota') as varchar(100)),1,23) as timestamp)", database="viamericas")
...
qryStr = f"(SELECT [BNKID],rtrim([DESC_TRANS_DIARIA1]) as DESC_TRANS_DIARIA1,[DEBIT_TRANS_DIARIA],[CONS_TRANS_DIARIA],rtrim([ID_GROUP_TRANS_DIARIA]) as ID_GROUP_TRANS_DIARIA,[BALANCE_TRANS_DIARIA],rtrim([DESCRIPCION_SUSPENSE]) as DESCRIPCION_SUSPENSE,[HOUR_TRANS_DIARIA],[LINK_REFERENCE],[DATE_TRANS_DIARIA],rtrim([FLAG_RECONCILIATION]) as FLAG_RECONCILIATION,[DATE_SYSTEM],[CREDIT_TRANS_DIARIA],rtrim([DES_TRANS_DIARIA]) as DES_TRANS_DIARIA,[WIRE_AMT_REFERENCED],[NUM_WIRETRANSFER],[CONS_TRANS_REVERSAL],[TOTAL_AMOUNT],rtrim([ID_CASHIER]) as ID_CASHIER,rtrim([ID_CONCEPTO_CONTABLE]) as ID_CONCEPTO_CONTABLE, convert(date, [DATE_TRANS_DIARIA]) as day FROM envio.dba.transaccion_diaria_payee WHERE [DATE_TRANS_DIARIA] >= '{athena_max_date}') x"
...
jdbcDF = spark.read.format('jdbc')\
    ...
    .option('partitionColumn', 'DATE_TRANS_DIARIA')\
...
if number_of_rows > 0:
    ...
    for day, count in thelist:
        if count > 0:
            ...
            totaldfpre = spark.sql(f" select t2.* from viamericas.transaccion_diaria_payee t2 where day = '{day}'")
            totaldfpre.unionByName(dfincremental).dropDuplicates()
  
            # Definir la ruta de salida en S3
            
            s3_output_path = f"s3://{secret_bucket_names['BUCKET_RAW']}/envio/dba/transaccion_diaria_payee/"
            # Escribir el DataFrame en formato Parquet en S3
            totaldfpre.write.partitionBy("day").parquet(s3_output_path, mode="overwrite")    
            print(f'Data for: {day} written succesfully')
            partition_creator_v2('viamericas','transaccion_diaria_payee', {'df': None, 'PartitionValues': tuple([str({day})])})
            abcdefg
else:
