...
max_date = get_partitions('viamericas','vcw_states_pricing')[0][0]
print("Max date in partitions is:", max_date)

# Get max date in athena

df = wr.athena.read_sql_query(sql=f"select coalesce(cast(max(UPDATE_DATE) as varchar), substring(cast(at_timezone(current_timestamp,'America/Bogota') as varchar(100)),1,23)) as max_date from viamericas.vcw_states_pricing where day = '{max_date}' and UPDATE_DATE <= cast(substring(cast(at_timezone(current_timestamp,'America/Bogota') as varchar(100)),1,23) as timestamp)", database="viamericas")
...
qryStr = f"(SELECT [ID_STATE_PRICING],[AGENCY_FIX_COMMN],[PROVIDER_INTERNAL_ID],[ID_DELIVERY_PROVIDER],[ID_BILLER],[PROVIDER_FEE],rtrim([ID_BRANCH]) as ID_BRANCH,rtrim([ID_FLAG_MODIFIED]) as ID_FLAG_MODIFIED,[CUSTOMER_FEE],[UPDATE_DATE],[VIAMERICAS_FEE],rtrim([ID_STATE]) as ID_STATE,[AGENCY_PCT_COMMN],[USERNAME], convert(date, [UPDATE_DATE]) as day FROM envio.dba.vcw_states_pricing WHERE [UPDATE_DATE] >= '{athena_max_date}') x"
...
jdbcDF = spark.read.format('jdbc')\
    ...
    .option('partitionColumn', 'UPDATE_DATE')\
...
if number_of_rows > 0:
    ...
    for day, count in thelist:
        if count > 0:
            ...
            totaldfpre = spark.sql(f" select t2.* from viamericas.vcw_states_pricing t2 where day = '{day}'")
            totaldfpre.unionByName(dfincremental).dropDuplicates()
  
            # Definir la ruta de salida en S3
            
            s3_output_path = f"s3://{secret_bucket_names['BUCKET_RAW']}/envio/dba/vcw_states_pricing/"
            # Escribir el DataFrame en formato Parquet en S3
            totaldfpre.write.partitionBy("day").parquet(s3_output_path, mode="overwrite")    
            print(f'Data for: {day} written succesfully')
            partition_creator_v2('viamericas','vcw_states_pricing', {'df': None, 'PartitionValues': tuple([str({day})])})
            abcdefg
else:
