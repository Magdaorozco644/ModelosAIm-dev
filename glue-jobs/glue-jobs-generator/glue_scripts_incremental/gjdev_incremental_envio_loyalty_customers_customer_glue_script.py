...
max_date = get_partitions('viamericas','customers_customer')[0][0]
print("Max date in partitions is:", max_date)

# Get max date in athena

df = wr.athena.read_sql_query(sql=f"select coalesce(cast(max(enrollment_date) as varchar), substring(cast(at_timezone(current_timestamp,'America/Bogota') as varchar(100)),1,23)) as max_date from viamericas.customers_customer where day = '{max_date}' and enrollment_date <= cast(substring(cast(at_timezone(current_timestamp,'America/Bogota') as varchar(100)),1,23) as timestamp)", database="viamericas")
...
qryStr = f"(SELECT [date_to_be_expired],rtrim([id]) as id,[initial_last_quarter_bill_pay],rtrim([product_id]) as product_id,[initial_remittances_per_quarter],[enrollment_date],[initial_last_quarter_top_ups],[date_of_first_transaction],[status_points],rtrim([teller_id]) as teller_id,[initial_top_ups_per_quarter],[total_points],[initial_last_quarter_remittances],[initial_bill_pay_per_quarter],[balance],[points_to_expire], convert(date, [enrollment_date]) as day FROM envio.loyalty.customers_customer WHERE [enrollment_date] >= '{athena_max_date}') x"
...
jdbcDF = spark.read.format('jdbc')\
    ...
    .option('partitionColumn', 'enrollment_date')\
...
if number_of_rows > 0:
    ...
    for day, count in thelist:
        if count > 0:
            ...
            totaldfpre = spark.sql(f" select t2.* from viamericas.customers_customer t2 where day = '{day}'")
            totaldfpre.unionByName(dfincremental).dropDuplicates()
  
            # Definir la ruta de salida en S3
            
            s3_output_path = f"s3://{secret_bucket_names['BUCKET_RAW']}/envio/loyalty/customers_customer/"
            # Escribir el DataFrame en formato Parquet en S3
            totaldfpre.write.partitionBy("day").parquet(s3_output_path, mode="overwrite")    
            print(f'Data for: {day} written succesfully')
            partition_creator_v2('viamericas','customers_customer', {'df': None, 'PartitionValues': tuple([str({day})])})
            abcdefg
else:
