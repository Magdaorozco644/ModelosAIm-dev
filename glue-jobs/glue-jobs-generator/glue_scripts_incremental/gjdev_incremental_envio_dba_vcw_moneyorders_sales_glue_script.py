...
qryStr = f"(SELECT [AGENCY_FEE],[AGENCY_PERCENTAGE],[C_TRANSACTION_DATE],[VIAMERICAS_FEE],rtrim([ID_PROVIDER]) as ID_PROVIDER,[ID_MONEYORDERS_SALES],[TOTAL],rtrim([STATUS]) as STATUS,[PROVIDER_TRANSACTION_DATE],[OPER_ID],[CUSTOMER_DISCOUNT_AMOUNT],[AGENCY_COMMISSION_BY_CHECK],[VOID_TRANSACTION_DATE],[VOID_OPER_ID],rtrim([ID_BRANCH]) as ID_BRANCH,[RP_TRANSACTION_ID],[AMOUNT],[LAST_UPDATED],[PROVIDER_FEE],[CUSTOMER_FEE],[TRANSACTION_DATE],[PROVIDER_TRANSACTION_ID],[FILE_USED], convert(date, [LAST_UPDATED]) as day FROM envio.dba.vcw_moneyorders_sales WHERE [LAST_UPDATED] >= '{athena_max_date}') x"
...
if number_of_rows > 0:
    ...
    for day, count in thelist:
        if count > 0:
            ...
            totaldfpre = spark.sql(f" select t2.* from viamericas.vcw_moneyorders_sales t2 where day = '{day}'")
            totaldfpre.unionByName(dfincremental).dropDuplicates()
  
            # Definir la ruta de salida en S3
            
            s3_output_path = f"s3://{secret_bucket_names['BUCKET_RAW']}/envio/dba/vcw_moneyorders_sales/"
            # Escribir el DataFrame en formato Parquet en S3
            totaldfpre.write.partitionBy("day").parquet(s3_output_path, mode="overwrite")    
            print(f'Data for: {day} written succesfully')
            partition_creator_v2('viamericas','vcw_moneyorders_sales', {'df': None, 'PartitionValues': tuple([str({day})])})
            abcdefg
else:
