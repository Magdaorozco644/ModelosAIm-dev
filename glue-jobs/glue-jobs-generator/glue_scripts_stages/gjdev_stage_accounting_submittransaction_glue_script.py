...
def thread_function(args):
    ...
    partition_creator_v2('stage','intermediate_accounting_submittransaction', {'df': None, 'PartitionValues': tuple([str(date)])})


def main(day):
    crawler_to_run = 'crw_stage_intermediate_accounting_submittransaction'
    ...
    s3_output_path = f"s3://{secret_bucket_names['BUCKET_STAGE']}/intermediate_accounting_submittransaction/"
    
    is_table_in_catalog = table_exists('stage', 'intermediate_accounting_submittransaction')

    if is_table_in_catalog:
        # Get date n months ago
        # date_before = day - timedelta(days=days_before)
        
        # print(f'accounting_submittransaction table exists. We will only update records between {date_before} and {day}')
        max_partition = get_partitions('stage','intermediate_accounting_submittransaction')[0][0]
        
        print("Max date in partitions is:", max_partition)
        
        # Get max date in athena
        df = wr.athena.read_sql_query(
            sql=f"select coalesce(cast(max(MODIFICATION_DATE) as varchar), substring(cast(at_timezone(current_timestamp,'US/Eastern') as varchar(100)),1,23)) as max_date from stage.intermediate_accounting_submittransaction where day = '{max_partition}' and MODIFICATION_DATE <= cast(substring(cast(at_timezone(current_timestamp,'US/Eastern') as varchar(100)),1,23) as timestamp)", 
            database="stage"
        )
        ...
        query_jdbcDF = f"select * from viamericas.accounting_submittransaction accounting_submittransaction where MODIFICATION_DATE >= '{athena_max_date}'"
        ...
        if number_of_rows > 0:
            ...
            with ThreadPoolExecutor(max_workers=10) as executor:
                ...
                for partition in thelist:
                    ...
                    query = f"select `(rank)?+.+` from (select accounting_submittransaction.*, row_number() over(partition by id order by COALESCE(MODIFICATION_DATE, '1900-01-01 00:00:00') desc) as rank from viamericas.accounting_submittransaction accounting_submittransaction where day = '{partition}' ) where rank=1"
        ...
        print(f'table accounting_submittransaction does not exists. We will update the whole accounting_submittransaction\'s history')
        
        query_jdbcDF = f"select `(rank)?+.+` from (select accounting_submittransaction.*, row_number() over(partition by id order by COALESCE(MODIFICATION_DATE, '1900-01-01 00:00:00') desc ) as rank from  viamericas.accounting_submittransaction accounting_submittransaction where day <= '{day}') where rank=1"
