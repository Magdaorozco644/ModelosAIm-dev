...
def thread_function(args):
    ...
    partition_creator_v2('stage','intermediate_sender', {'df': None, 'PartitionValues': tuple([str(date)])})


def main(day):
    crawler_to_run = 'crw_stage_intermediate_sender'
    ...
    if is_table_in_catalog:
        ...
        query_jdbcDF = f"select * from viamericas.sender sender where LAST_UPDATED >= '{athena_max_date}'"
        ...
        if number_of_rows > 0:
            ...
            with ThreadPoolExecutor(max_workers=10) as executor:
                ...
                for partition in thelist:
                    ...
                    query = f"select `(rank)?+.+` from (select sender.*, row_number() over(partition by ID_BRANCH, ID_SENDER order by COALESCE(LAST_UPDATED, '1900-01-01 00:00:00') desc) as rank from viamericas.sender sender where day = '{partition}' ) where rank=1"
        ...
        print(f'table sender does not exists. We will update the whole sender\'s history')
        
        query_jdbcDF = f"select `(rank)?+.+` from (select sender.*, row_number() over(partition by ID_BRANCH, ID_SENDER order by COALESCE(LAST_UPDATED, '1900-01-01 00:00:00') desc ) as rank from  viamericas.sender sender where day <= '{day}') where rank=1"
