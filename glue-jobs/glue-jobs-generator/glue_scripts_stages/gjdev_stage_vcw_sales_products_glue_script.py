...
def thread_function(args):
    ...
    partition_creator_v2('stage','intermediate_vcw_sales_products', {'df': None, 'PartitionValues': tuple([str(date)])})


def main(day):
    crawler_to_run = 'crw_stage_intermediate_vcw_sales_products'
    ...
    s3_output_path = f"s3://{secret_bucket_names['BUCKET_STAGE']}/intermediate_vcw_sales_products/"
    
    is_table_in_catalog = table_exists('stage', 'intermediate_vcw_sales_products')

    if is_table_in_catalog:
        # Get date n months ago
        # date_before = day - timedelta(days=days_before)
        
        # print(f'vcw_sales_products table exists. We will only update records between {date_before} and {day}')
        max_partition = get_partitions('stage','intermediate_vcw_sales_products')[0][0]
        
        print("Max date in partitions is:", max_partition)
        
        # Get max date in athena
        df = wr.athena.read_sql_query(
            sql=f"select coalesce(cast(max(LAST_UPDATED) as varchar), substring(cast(at_timezone(current_timestamp,'US/Eastern') as varchar(100)),1,23)) as max_date from stage.intermediate_vcw_sales_products where day = '{max_partition}' and LAST_UPDATED <= cast(substring(cast(at_timezone(current_timestamp,'US/Eastern') as varchar(100)),1,23) as timestamp)", 
            database="stage"
        )
        ...
        query_jdbcDF = f"select * from viamericas.vcw_sales_products vcw_sales_products where LAST_UPDATED >= '{athena_max_date}'"
        ...
        if number_of_rows > 0:
            ...
            with ThreadPoolExecutor(max_workers=10) as executor:
                ...
                for partition in thelist:
                    ...
                    query = f"select `(rank)?+.+` from (select vcw_sales_products.*, row_number() over(partition by SALESID order by COALESCE(LAST_UPDATED, '1900-01-01 00:00:00') desc) as rank from viamericas.vcw_sales_products vcw_sales_products where day = '{partition}' ) where rank=1"
        ...
        print(f'table vcw_sales_products does not exists. We will update the whole vcw_sales_products\'s history')
        
        query_jdbcDF = f"select `(rank)?+.+` from (select vcw_sales_products.*, row_number() over(partition by SALESID order by COALESCE(LAST_UPDATED, '1900-01-01 00:00:00') desc ) as rank from  viamericas.vcw_sales_products vcw_sales_products where day <= '{day}') where rank=1"
