...
def thread_function(args):
    ...
    partition_creator_v2('stage','intermediate_sf_safe_transactions', {'df': None, 'PartitionValues': tuple([str(date)])})


def main(day):
    crawler_to_run = 'crw_stage_intermediate_sf_safe_transactions'
    ...
    s3_output_path = f"s3://{secret_bucket_names['BUCKET_STAGE']}/intermediate_sf_safe_transactions/"
    
    is_table_in_catalog = table_exists('stage', 'intermediate_sf_safe_transactions')

    if is_table_in_catalog:
        # Get date n months ago
        # date_before = day - timedelta(days=days_before)
        
        # print(f'sf_safe_transactions table exists. We will only update records between {date_before} and {day}')
        max_partition = get_partitions('stage','intermediate_sf_safe_transactions')[0][0]
        
        print("Max date in partitions is:", max_partition)
        
        # Get max date in athena
        df = wr.athena.read_sql_query(
            sql=f"select coalesce(cast(max(TRANSACTION_DATE) as varchar), substring(cast(at_timezone(current_timestamp,'US/Eastern') as varchar(100)),1,23)) as max_date from stage.intermediate_sf_safe_transactions where day = '{max_partition}' and TRANSACTION_DATE <= cast(substring(cast(at_timezone(current_timestamp,'US/Eastern') as varchar(100)),1,23) as timestamp)", 
            database="stage"
        )
        ...
        query_jdbcDF = f"select * from viamericas.sf_safe_transactions sf_safe_transactions where TRANSACTION_DATE >= '{athena_max_date}'"
        ...
        if number_of_rows > 0:
            ...
            with ThreadPoolExecutor(max_workers=10) as executor:
                ...
                for partition in thelist:
                    ...
                    query = f"select `(rank)?+.+` from (select sf_safe_transactions.*, row_number() over(partition by ID_SAFE_TRANSACTIONS order by COALESCE(TRANSACTION_DATE, '1900-01-01 00:00:00') desc) as rank from viamericas.sf_safe_transactions sf_safe_transactions where day = '{partition}' ) where rank=1"
        ...
        print(f'table sf_safe_transactions does not exists. We will update the whole sf_safe_transactions\'s history')
        
        query_jdbcDF = f"select `(rank)?+.+` from (select sf_safe_transactions.*, row_number() over(partition by ID_SAFE_TRANSACTIONS order by COALESCE(TRANSACTION_DATE, '1900-01-01 00:00:00') desc ) as rank from  viamericas.sf_safe_transactions sf_safe_transactions where day <= '{day}') where rank=1"
