...
df = wr.athena.read_sql_query(sql="select cast(max(processeddate) as varchar) as max_date from viamericas.historicalonholdrelease", database="viamericas")
...
qryStr = f"(SELECT [ProcessedDate],[HoldDate],[idVoidSubReason],[idVoidReason],[CheckStatus],[Review],[CheckId],[IdHoldGroup],[ReviewDate],[ProcessedUser],[HistoricalId] FROM viachecks.viacheck.historicalonholdrelease WHERE processeddate > '{athena_max_date}') x"
