...
df = wr.athena.read_sql_query(sql="select cast(max(system_date) as varchar) as max_date from viamericas.checkreader_score", database="viamericas")
...
qryStr = f"(SELECT [CDCHEQUE],[SCORE],[CHECK_SCORE_ID],[CHECKID],[CHECKPROPERTYID],[CDMAKER],[SYSTEM_DATE],[VCLCHECKBATCHID],[RESULT] FROM viachecks.dbo.checkreader_score WHERE system_date > '{athena_max_date}') x"
