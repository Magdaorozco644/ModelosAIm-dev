...
df = wr.athena.read_sql_query(sql="select cast(max(FEED_DATE) as varchar) as max_date from viamericas.forex_feed_market", database="viamericas")
...
qryStr = f"(SELECT [FEED_ID],[SYMBOL],[FEED_TIME],[FEED_SOURCE],[FEED_PRICE],[FEED_DATE],[FEED_ASK],[FEED_REMAINING],[FEED_BID] FROM envio.dba.forex_feed_market WHERE FEED_DATE > '{athena_max_date}') x"
