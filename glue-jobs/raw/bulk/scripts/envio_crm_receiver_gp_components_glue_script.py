...
def main(dates):
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = []
        for date in dates:
            qryStr = f"(SELECT [ID_BRANCH] ,[VIADEAL_REAL] ,[DATE_RECEIVER] ,[ID_RECEIVER] ,[VIADEAL_ESTIMATED] FROM envio.crm.receiver_gp_components WHERE DATE_RECEIVER >= '{date}-01-01 00:00:00.000' AND DATE_RECEIVER <= '{date}-12-31 23:59:59.000') x"
