...
def main(dates):
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = []
        for date in dates:
            qryStr = f"([checkID] ,[insertedDate] ,[FEE] ,[returnID] ,[returnTime] ,[SNENVIO] ,[CONS_TRANS_DIARIA] ,[creditTransactionId] ,[returnReason] ,[errorIRD] ,[reprocessIRD] FROM viachecks.dbo.returnchecks WHERE insertedDate >= '{date}-01-01 00:00:00.000' AND insertedDate <= '{date}-12-31 23:59:59.000') x"
