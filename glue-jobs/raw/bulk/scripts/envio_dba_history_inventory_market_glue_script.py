...
def main(dates):
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = []
        for date in dates:
            qryStr = f"(SELECT [DOLLAR] ,[viatasa_agencia] ,[INFO_TYPE] ,[AVG_RATE] ,[DATE] ,[NUM_ORDERS] ,[GROUP_TYPE] ,[ID_CURRENCY] ,[PESOS] ,[NEW_PURCHASE] ,[viatasa_via] FROM envio.dba.history_inventory_market) x"
            # create arguments
