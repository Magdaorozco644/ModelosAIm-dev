...
def main(dates):
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = []
        for date in dates:
            qryStr = f"(SELECT [DATE_SYSTEM] ,[ID_CONCEPTO_CONTABLE] ,[HOUR_TRANS_DIARIA] ,[ID_GROUP_TRANS_DIARIA] ,[CREDIT_TRANS_DIARIA] ,[CONS_TRANS_REVERSAL] ,[NUM_WIRETRANSFER] ,[DESC_TRANS_DIARIA1] ,[BNKID] ,[TOTAL_AMOUNT] ,[LINK_REFERENCE] ,[BALANCE_TRANS_DIARIA] ,[DEBIT_TRANS_DIARIA] ,[ID_CASHIER] ,[DES_TRANS_DIARIA] ,[CONS_TRANS_DIARIA] ,[DESCRIPCION_SUSPENSE] ,[DATE_TRANS_DIARIA] ,[WIRE_AMT_REFERENCED] ,[FLAG_RECONCILIATION] FROM envio.dba.transaccion_diaria_payee WHERE DATE_TRANS_DIARIA >= '{date}-01-01 00:00:00.000' AND DATE_TRANS_DIARIA <= '{date}-12-31 23:59:59.000') x"
