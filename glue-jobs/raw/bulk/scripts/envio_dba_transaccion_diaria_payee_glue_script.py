...
def main(dates):
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = []
        for date in dates:
            qryStr = f"(SELECT [FLAG_RECONCILIATION] ,[DES_TRANS_DIARIA] ,[DEBIT_TRANS_DIARIA] ,[BALANCE_TRANS_DIARIA] ,[DESC_TRANS_DIARIA1] ,[BNKID] ,[ID_CONCEPTO_CONTABLE] ,[CREDIT_TRANS_DIARIA] ,[DATE_TRANS_DIARIA] ,[LINK_REFERENCE] ,[CONS_TRANS_REVERSAL] ,[WIRE_AMT_REFERENCED] ,[CONS_TRANS_DIARIA] ,[DATE_SYSTEM] ,[TOTAL_AMOUNT] ,[ID_CASHIER] ,[HOUR_TRANS_DIARIA] ,[NUM_WIRETRANSFER] ,[DESCRIPCION_SUSPENSE] ,[ID_GROUP_TRANS_DIARIA] FROM envio.dba.transaccion_diaria_payee WHERE DATE_TRANS_DIARIA >= '{date}-01-01 00:00:00.000' AND DATE_TRANS_DIARIA <= '{date}-12-31 23:59:59.000') x"
