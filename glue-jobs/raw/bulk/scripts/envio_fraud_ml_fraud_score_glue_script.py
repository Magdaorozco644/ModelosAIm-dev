...
def main(dates):
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = []
        for date in dates:
            qryStr = f"(SELECT [WAS_FRAUD_OLD] ,[MATRIX_CATEGORY] ,[ID] ,[DATE_PROCESSED] ,[ID_BRANCH] ,[IS_PROCESSING] ,[WAS_FRAUD] ,[SCORE_OLD] ,[IS_AT_VECTOR] ,[DATE_RECEIVER] ,[IS_PROCESSED] ,[ID_RECEIVER] ,[SCORE_2] ,[SCORE] ,[SCORE_1] ,[DATE_CREATED] FROM envio.fraud.ml_fraud_score WHERE DATE_CREATED >= '{date}-01-01 00:00:00.000' AND DATE_CREATED <= '{date}-12-31 23:59:59.000') x"
