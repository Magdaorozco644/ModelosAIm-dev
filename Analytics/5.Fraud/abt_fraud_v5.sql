...
create table analytics.abt_fraud_temp as (
        with receiver_fraud_fecha as(
            ...
            and r.id_flag_receiver not in('A','C')
        ),
        ...
        location_nro_fraud_cte as (
            ...
            (select fr.*, trim(br.id_location) as id_location
                ...
                on fr.id_branch = br.id_branch and br.id_main_branch not like 'TS%' and br.id_flag_branch ='A') brfr
...
select id_receiver, id_branch, date_receiver, id_sender, net_amount_receiver, mode_pay_receiver, id_country_receiver, id_state_receiver, id_payment, id_city_receiver, bank_receiver, id_flag_receiver
from viamericas.receiver where date_receiver between cast('2023-01-01' as timestamp) and cast('2024-02-01' as timestamp)
and iD_FLAG_RECEIVER not in('A','C')
union
select try(cast(trim(id_receiver) as integer)), id_branch, try(cast(trim(date_receiver) as timestamp)) , try(cast(trim(id_sender) as integer)),
        try(cast(trim(net_amount_receiver) as double)) , mode_pay_receiver, id_country_receiver, id_state_receiver, id_payment, id_city_receiver, bank_receiver, iD_FLAG_RECEIVER
from viamericas.test_receiver
where iD_FLAG_RECEIVER not in('A','C');
...
union
select try(cast(trim(id_receiver) as integer)), trim(id_branch) as id_branch, fraud_classification, fraud_type
from viamericas.test_receiver_fraud;

--Creo nueva tabla sender con datos actualizado
create table analytics.sender_large as
select id_sender, id_sender_global, state_sender, id_country, trim(id_branch) as id_branch, id_state, day
from viamericas.sender
union
select try(cast(id_sender as integer)), try(cast(id_sender_global as integer)), state_sender, id_country, trim(id_branch), id_state, last_updated
...
create table analytics.abt_fraud_2024 as (
        select abt.*
        from analytics.abt_fraud_temp abt inner join
            (   select id_receiver, id_branch, max(try(cast(day as timestamp))) last
                from analytics.abt_fraud_temp where (date_receiver >= try(cast(day as timestamp)) or day = 'NULL')
                group by id_receiver, id_branch
            ) ult_st
        on abt.id_receiver = ult_st.id_receiver and abt.id_branch = ult_st.id_branch
        and (try(cast(abt.day as timestamp)) = ult_st.last or abt.day is NULL or abt.day = 'NULL')
);
